...
public class BaseImageDecoder implements ImageDecoder {
	...
	public Bitmap decode(ImageDecodingInfo decodingInfo) throws IOException {
		...
		if (decodedBitmap == null) {
			Log.e(ERROR_CANT_DECODE_IMAGE+"", decodingInfo.getImageKey());
		} else {
	...
	protected ExifInfo defineExifOrientation(String imageUri, String mimeType) {
		int rotation = 0;
		boolean flip = false;
		if ("image/jpeg".equalsIgnoreCase(mimeType) &&ImageDownloader.Scheme.ofUri(imageUri) == ImageDownloader.Scheme.FILE) {
			try {
				...
				Log.w("Can't read EXIF tags from file [%s]", imageUri);
			}
		}
	...
	protected Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
		...
		if (scaleType != ImageScaleType.NONE) {
			...
			Log.i(LOG_SABSAMPLE_IMAGE, imageSize+"-"+ imageSize.scaleDown(scale)+"-"+ scale+"-"+ decodingInfo.getImageKey());
		}
	...
	protected Bitmap considerExactScaleAndOrientaiton(Bitmap subsampledBitmap, ImageDecodingInfo decodingInfo, int rotation, boolean flipHorizontal) {
		...
		if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
			...
			if (Float.compare(scale, 1f) != 0) {
				...
				if (loggingEnabled) Log.i(LOG_SCALE_IMAGE+"", srcSize+""+srcSize.scale(scale)+""+scale+""+decodingInfo.getImageKey());
			}
		}
		// Flip bitmap if need
		if (flipHorizontal) {
			...
			if (loggingEnabled) Log.i(LOG_FLIP_IMAGE+"", decodingInfo.getImageKey());
		}
		// Rotate bitmap if need
		if (rotation != 0) {
			m.postRotate(rotation);

			if (loggingEnabled) Log.i(LOG_ROTATE_IMAGE+"", rotation+""+ decodingInfo.getImageKey());
		}
