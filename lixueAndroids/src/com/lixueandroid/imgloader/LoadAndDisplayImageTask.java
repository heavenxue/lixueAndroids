...
import com.lixueandroid.imgloader.FailReason.FailType;
import com.lixueandroid.imgloader.ImageDownloader.Scheme;
...
final class LoadAndDisplayImageTask implements Runnable {
		...
		try {
			...
			if (bmp == null) {
				...
				if (options.shouldPreProcess()) {
					...
					if (bmp == null) {
						Log.w("",WARNING_PRE_PROCESSOR_NULL);
					}
				}
			...
			if (bmp != null && options.shouldPostProcess()) {
				...
				if (bmp == null) {
					Log.w(WARNING_POST_PROCESSOR_NULL, memoryCacheKey);
				}
			}
		} finally {
	...
	private void log(String message) {
		if (loggingEnabled) Log.i(message, memoryCacheKey);
	}

	private void log(String message, Object... args) {
		if (loggingEnabled) Log.i(message, args.toString());
	}
}