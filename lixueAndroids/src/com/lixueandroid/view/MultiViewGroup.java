...
import com.lixueandroid.activity.MultiScreenActivity;
...
// measure过程
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			// 设置每个子视图的大小 ， 即全屏
			child.measure(getWidth(), MultiScreenActivity.screenHeight);
		}
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if(child.getVisibility() != View.GONE)
			    child.layout(startLeft, startTop, startLeft + getWidth(), startTop + MultiScreenActivity.screenHeight );
	...
	public void stopMove(){
		...
		if(mScroller != null){
			...
			if(!mScroller.isFinished()){
				...
				Log.i(TAG, "当前滑动到了："+scrollCurX);
				int descScreen = ( scrollCurX +  getWidth() / 2) /  getWidth() ;
				Log.i(TAG, "当前滑动到第"+descScreen+"屏");
...
//				Log.i(TAG, "----mScroller.is not finished ---- scrollCurX " + scrollCurX);
	...
	private int mTouchState = TOUCH_STATE_REST;//当前的手触状态
	//-------------------------- 
	//处理触摸事件 ~
	public static int  SNAP_VELOCITY = 600 ;//触摸速率
	private int mTouchSlop = 0 ;//最小滑动距离
	private float mLastionMotionX = 0 ;
	private float mLastMotionY = 0 ;
	//处理触摸的速率，用来追踪触摸事件（flinging事件和其他手势事件）的速率
	...
	public boolean onTouchEvent(MotionEvent event){
		...
		case MotionEvent.ACTION_MOVE:
			...
			scrollBy(detaX, 0);
			Log.e(TAG, "--- MotionEvent.ACTION_MOVE--> detaX is 手指滑动的距离： " + detaX );
			mLastionMotionX = x ;
			mLastMotionY=y;
			break ;
		case MotionEvent.ACTION_UP:
			...
			Log.e(TAG , "---velocityX---当前滑动完毕后的速率：" + velocityX);
