	...
	class ReloadImageThread extends Thread {
		...
		public void run() {
			if (flowTag != null) {
				...
				try {
					buf = new BufferedInputStream(flowTag.getAssetManager().open(flowTag.getFileName()));
				...
				((Activity) context).runOnUiThread(new Runnable() {
					public void run() {
						if (bitmap != null) {// 此处在线程过多时可能为null
							...
							if (lp == null) {
								lp = new LayoutParams(flowTag.getItemWidth(),layoutHeight);
							}
							setLayoutParams(lp);
							setImageBitmap(bitmap);
							Handler h = getViewHandler();
							Message m = h.obtainMessage(flowTag.what, width,layoutHeight, FlowView.this);
							h.sendMessage(m);
						}
					}
				});
